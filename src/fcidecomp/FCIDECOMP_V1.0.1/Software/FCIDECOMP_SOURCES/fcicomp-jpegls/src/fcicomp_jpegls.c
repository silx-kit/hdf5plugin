// $Id: fcicomp_jpegls.c 778 2016-03-09 07:56:29Z delaunay $
// =============================================================
//
// PROJECT : FCICOMP
//
...
/* Declare an object of type jmp_buf hold the state information to be
 * restored by a non-local exit, when a segmentation fault is caught */
static jmp_buf exitJmp;

///* Define a segmentation fault handler to use when CharLS crashes with a SIGSEGV */
void segFaultHandler(int sig) {
	/* Print an error */
	LOG(ERROR_SEVERITY, SEGFAULT_CAUGHT);
	/* Jump back to the line "if (setjmp(exitJmp) == SIGSEGV)" */
	longjmp(exitJmp, sig);
}
...
//		/* The bit-depth and transform used are not supported by CharLS */
//		fcicompJlsErr = FJLS_UNSUPPORTED_BITDEPTH_FOR_TRANSFORM;
//		break;
//	case UnsupportedColorTransform:
//		/* The color transform used is not supported by CharLS */
//		fcicompJlsErr = FJLS_UNSUPPORTED_COLOR_TRANSFORM;
//		break;
	default:
...
//	case UnsupportedBitDepthForTransform:
//		/* The bit-depth and transform used are not supported by CharLS */
//		msg = UNSUPPORTED_BIT_DEPTH_FOR_TRANSFORM_MSG;
//		break;
//	case UnsupportedColorTransform:
//		/* The color transform used is not supported by CharLS */
//		msg = UNSUPPORTED_COLOR_TRANSFORM_MSG;
//		break;
	default:
		/* Default: Unknown CharLS error code */
...
int jpeglsCompress(void *outBuf, size_t outBufSize, size_t *compressedSize, const void *inBuf, size_t inBufSize,
	...
	if (jlsParams.components > MAX_COMPONENTS) {
		...
		/* Set the segmentation fault handler */
		signal(SIGSEGV, segFaultHandler);

		/* If a segmentation fault is caught */
		if (setjmp(exitJmp) == SIGSEGV) {
			/* Set the compressed size to 0 */
			*compressedSize = 0;
			/* Return an error code */
			result = FJLS_SEGFAULT_CAUGHT;
		} else {
		...
		} /* end if a segmentation fault is not caught */

		/* Restore default action for the SIGSEGV signal */
